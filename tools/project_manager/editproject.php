...
class ProjectInfoHolder
{
    ...
    public function set_from_nothing()
    {
        ...
        $this->charsuites = SiteConfig::$default_project_char_suites;
    }
    ...
    public function show_visible_controls()
    {
        ...
        if (user_is_a_sitemanager()) {
            ...
            $this->row(_("Project Manager"), 'DP_user_field', $this->project->username, 'username', sprintf(_("%s username only."), SiteConfig::$site_abbreviation), ["required" => true]);
        }
        ...
        if ($can_edit_PPer) {
            $this->row(_("PPer/PPVer"), 'DP_user_field', $this->project->checkedoutby, 'checkedoutby', sprintf(_("Optionally reserve for a PPer. %s username only."), SiteConfig::$site_abbreviation));
        } else {
        ...
        $this->row(_("Image Preparer"), 'DP_user_field', $this->project->image_preparer, 'image_preparer', sprintf(_("%s user who scanned or harvested the images."), SiteConfig::$site_abbreviation));
        $this->row(_("Text Preparer"), 'DP_user_field', $this->project->text_preparer, 'text_preparer', sprintf(_("%s user who prepared the text files."), SiteConfig::$site_abbreviation));
        $this->row(

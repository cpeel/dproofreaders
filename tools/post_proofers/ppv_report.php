...
if ($action == SHOW_BLANK_ENTRY_FORM) {
    ...
    $ppbit = sprintf($ppbita, $project->postproofer, $pguser, $nameofwork, SiteConfig::$site_name);
    ...
    $ppvbit = sprintf($ppvbita, $pguser, $nameofwork, SiteConfig::$general_help_email_addr, SiteConfig::$site_name);
    $ppvbit = ppv_report_wrap($ppvbit);

    if (isset($_POST['cc_pp'])) {
        $to = $pper->email;
        $subject = SiteConfig::$site_abbreviation . " PP: $nameofwork";
    ...
    if (isset($_POST['cc_ppv'])) {
        $to = $ppver->email;
        $subject = SiteConfig::$site_abbreviation . " PPV: $nameofwork";
        $message = $ppvbit . $reportcard;
        send_mail($to, $subject, $message);
    }

    $to = SiteConfig::$ppv_reporting_email_addr;

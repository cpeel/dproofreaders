...
function api_rate_limit($key)
{
    ...
    if ($expire_time === false || $expire_time < time()) {
        $expire_time = time() + SiteConfig::$api_rate_limit_seconds_in_window;
    ...
    if ($count > SiteConfig::$api_rate_limit_requests_per_window) {
        throw new RateLimitExceeded(
            "Rate limit exceeded, resets in $seconds_before_reset seconds"
        );
    }

    $requests_left_before_reset = SiteConfig::$api_rate_limit_requests_per_window - $count;
    header("X-Rate-Limit-Limit: " . SiteConfig::$api_rate_limit_requests_per_window);
